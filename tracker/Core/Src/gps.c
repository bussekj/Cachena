...
{
//    APP_LOG(TS_OFF, VLEVEL_H, "GPS OUTPUT: %s\r\n", msg);
}
 ...
 */
int split_fields(char *buf, char **fields, int max_fields)
{
    int count = 0;
    char *p = buf;
 ...
 */
bool parse_lat(const char *raw, const char *hemi, double *out)
{
 ...
 */
bool parse_lon(const char *raw, const char *hemi, double *out)
{
    if (!raw || !*raw || !hemi || !*hemi) return false;
    double raw_val = atof(raw);
    int    deg  = (int)(raw_val / 100);
...
GPS_Message_Queue_t handle_gll(char **f, int nf)
{
    ...
    gpsdata.latitude = g.coord.lat_valid ? (int)g.coord.latitude  : 0;
    gpsdata.longitude = g.coord.lon_valid ? (int)g.coord.longitude : 0;
    gpsdata.batteryLevel = 50;
    gpsdata.isValid = g.valid;
    gpsdata.trackerId = 1;
...
typedef struct {
    const char *id;
    GPS_Message_Queue_t (*handler)(char **fields, int nfields);
} NmeaDispatch;
