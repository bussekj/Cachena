...
#define NMEA_MAX_LEN     128   /* Maximum length of one NMEA sentence       */
#define NMEA_MAX_FIELDS   32   /* Maximum comma-separated fields per sentence*/
#define NMEA_MAX_SVS      16   /* Maximum satellite entries in a GSV message */
...
typedef struct {
    char    utc_time[16];
    NmeaCoord coord;
    uint8_t fix_quality;    /* 0=no fix, 1=GPS SPS, 2=DGPS, 6=DR */
    uint8_t satellites;
    float   hdop;
    float   altitude_m;
    float   geoid_sep_m;
...
typedef struct {
    ...
    uint8_t mode2;          /* 1=NoFix, 2=2D, 3=3D       */
    uint8_t svs[12];
    uint8_t sv_count;
    float   pdop;
    float   hdop;
...
/* -------------------------------------------------------------------------
 * Output / callback — implement this for your platform
 * ---------------------------------------------------------------------- */

/**
 * nmea_output() – called with a formatted, human-readable string after each
 * successfully parsed sentence.  Replace with printf, UART TX, syslog, etc.
 */
static void nmea_output(const char *msg)
{
    printf("%s\n", msg);
}
 ...
 */
void nmea_feed_byte(uint8_t byte)
{
    char c = (char)byte;
...
/* =========================================================================
 * POSIX demo — reads from a real or simulated serial port (stdin / file).
 * Compile with -DPOSIX_DEMO to build a standalone executable.
 *
 *   gcc -DPOSIX_DEMO -o nmea_parser nmea_parser.c
 *   cat nmea_log.txt | ./nmea_parser
 * ================================================================= */
...
static int open_serial(const char *port, int baud_flag)
{
    int fd = open(port, O_RDONLY | O_NOCTTY | O_SYNC);
    if (fd < 0) {
        perror("open");
        return -1;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2) {
        /* Try to open the supplied serial port at 9600 baud */
        fd = open_serial(argv[1], B9600);
        if (fd < 0) {
            fprintf(stderr, "Falling back to stdin.\n");
            fd = STDIN_FILENO;
        } else {
            printf("Opened %s at 9600 baud.\n", argv[1]);
        }
    } else {
        printf("No port supplied — reading from stdin.\n");
        fd = STDIN_FILENO;
    }

    printf("Listening for NMEA sentences. Press Ctrl+C to stop.\n\n");
    ...
    uint32_t total, valid_gll;
    nmea_get_stats(&total, &valid_gll);
    printf("\nTotal sentences parsed : %lu\n", (unsigned long)total);
    printf("Valid GLL reads        : %lu\n",  (unsigned long)valid_gll);
